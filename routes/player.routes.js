...
router.get("/", async (req, res) => {
  try {
    ...
    const players = await Sample.find()
      .limit(limit)
      .skip((page - 1) * limit)
      .populate("child");
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const player = await Player.findById(id).populate("child");
    if (player) {
      res.json(player);
    } else {
      res.status(404).json({});
...
router.get("/title/:title", async (req, res) => {
  ...
  try {
    const player = await Player.find({ title: new RegExp("^" + title.toLowerCase(), "i") }).populate("child");
    if (player?.length) {
      res.json(player);
    } else {
      res.status(404).json([]);
